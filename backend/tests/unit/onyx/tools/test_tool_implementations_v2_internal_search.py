...
def test_internal_search_core_no_results(
    fake_run_context: RunContextWrapper[ChatTurnContext],
    fake_session_context_manager: FakeSessionContextManager,
):
    """Test _internal_search_core with no search results"""
    # Arrange
    query = "test search query"
    test_pipeline = create_fake_search_pipeline_empty()
    ...
    # Check the types of emitted events (no document delta since no results)
    assert isinstance(emitter.emitted_events[0].obj, SearchToolStart)
    assert isinstance(emitter.emitted_events[1].obj, SearchToolDelta)
    assert isinstance(emitter.emitted_events[2].obj, SectionEnd)


def test_internal_search_core_disconnected(
    fake_run_context: RunContextWrapper[ChatTurnContext],
    fake_session_context_manager: FakeSessionContextManager,
):
    """Test _internal_search_core when connection is lost"""
    from unittest.mock import patch
    ...
    # Create a pipeline with multiple responses to test the connection check
    # The first response is not the SEARCH_RESPONSE_SUMMARY_ID, so the loop continues
    # The second response is the SEARCH_RESPONSE_SUMMARY_ID, but by then the connection check should break
    test_sections = [create_fake_inference_section()]
    responses = [
        FakeSearchResponse(response_id="other_response_id", top_sections=[]),
        FakeSearchResponse(
    ...
    test_pipeline = FakeSearchPipeline(responses=responses)

    # Create a Redis client that simulates a disconnected state
    # by having the fence key exist (which means stop signal is set)
    disconnected_redis = FakeRedis()
    # Set a fence key to simulate the stop signal being set
    # The fence key format is: chatsessionstop_fence_{tenant_id}_{chat_session_id}
    chat_session_id = (
        fake_run_context.context.run_dependencies.dependencies_to_maybe_remove.chat_session_id
    )
    fence_key = f"chatsessionstop_fence_test_tenant_{chat_session_id}"
    disconnected_redis.set(fence_key, 0)

    # Verify that is_connected returns False with our setup
    with patch(
        "onyx.chat.stop_signal_checker.get_current_tenant_id",
        return_value="test_tenant",
    ):
        assert not is_connected(chat_session_id, disconnected_redis)
    ...
    with patch(
        ...
        result = run_internal_search_core_with_dependencies(
            ...
            query,
            test_pipeline,
            fake_session_context_manager,
            redis_client=disconnected_redis,
        )

    # Assert
    assert result == []

    # Verify emitter events were captured (should break before processing results)
    emitter = fake_run_context.context.run_dependencies.emitter
    assert len(emitter.emitted_events) == 3
...
def test_internal_search_core_exception_handling(
    fake_run_context: RunContextWrapper[ChatTurnContext],
    fake_session_context_manager: FakeSessionContextManager,
):
    """Test that _internal_search_core handles exceptions properly"""
    # Arrange
    query = "test search query"
    test_pipeline = create_fake_search_pipeline_with_results(
    ...
    # Act & Assert
    with pytest.raises(Exception, match="Test exception from search pipeline"):
        run_internal_search_core_with_dependencies(
            fake_run_context, query, test_pipeline, fake_session_context_manager
        )
...
def test_internal_search_core_none_pipeline(
    fake_run_context: RunContextWrapper[ChatTurnContext],
    fake_session_context_manager: FakeSessionContextManager,
):
    """Test that _internal_search_core handles None pipeline properly"""
    # Arrange
    query = "test search query"

    # Act & Assert
    with pytest.raises(RuntimeError, match="Search tool not available in context"):
        run_internal_search_core_with_dependencies(
            fake_run_context, query, None, fake_session_context_manager
        )
...
def test_internal_search_core_multiple_responses(
    fake_run_context: RunContextWrapper[ChatTurnContext],
    fake_session_context_manager: FakeSessionContextManager,
):
    """Test _internal_search_core with multiple tool responses"""
    # Arrange
    query = "test search query"
    test_pipeline = create_fake_search_pipeline_multiple_responses()
    ...
    assert (
        len(result) == 1
    )  # Should only return the sections from the SEARCH_RESPONSE_SUMMARY_ID response

    # Verify only one iteration answer was created (from the first SEARCH_RESPONSE_SUMMARY_ID)
    assert (
        len(fake_run_context.context.aggregated_context.global_iteration_responses) == 1
    )
